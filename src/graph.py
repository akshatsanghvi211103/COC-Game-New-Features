...
def moveWithoutBreakingWalls(grid,start):
    ...
    if len(final) == 0:
        return None
    # so they are doing BFS from each of the options and then finding the min value at each iteration ig
    # find key with min value, where key is the tuple of coordinates
    for key in final:
