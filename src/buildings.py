import numpy as np
import points as pt
from characters import stealtharchers, barbarians, dragons, balloons, archers, healers, kingArr
import random
...
class Cannon(Building):
    ...
    def scan_for_targets(self, King):
        ...
        # we dont care about stealtharchers while they are invisible, as cannons can attack only on troop at a time...this aint true, while attack target, we are attacking all of the troops in the range
        ...
        for stealtharcher in stealtharchers:
            if (stealtharcher.invisible == False) and ((stealtharcher.position[0] - self.position[0])**2 + (stealtharcher.position[1] - self.position[1])**2 <= self.attack_radius**2):
                self.isShooting = True
                self.attack_target(stealtharcher)
                return
...
class Wall(Building):
    def __init__(self, position, V):
        ...
        self.type = 'wall'
        self.destroy_radius = 2
        self.explode_damage = 200
    
    def destroy(self):
        if self.level >= 3:
            troops = barbarians + archers + stealtharchers + kingArr
            for troop in troops:
                if abs(troop.position[0] - self.position[0]) <= self.destroy_radius and abs(troop.position[1] - self.position[1]) <= self.destroy_radius:
                    troop.deal_damage(self.explode_damage)
...
class WizardTower(Building):
    ...
    def scan_for_targets(self, King):
        ...
        for stealtharcher in stealtharchers:
            if (stealtharcher.invisible == False) and ((stealtharcher.position[0] - self.position[0])**2 + (stealtharcher.position[1] - self.position[1])**2 <= self.attack_radius**2):
                self.isShooting = True
                self.attack_target(stealtharcher, 0)
                return
