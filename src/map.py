...
def stealtharchColor(stealtharch):
    health = stealtharch.health
    max_health = stealtharch.max_health
    percentage = (health*100)/max_health
    if(percentage > 50):
        return Back.LIGHTMAGENTA_EX
    elif percentage > 20:
...
def printMap(V):
    ...
    for stealtharch in stealtharchers:
        ...
        # it is fine if player is shown a visible stealth archer
        map_matrix[a][b] = stealtharchColor(stealtharch) + Fore.BLACK + 'ST'
        map_matrix[a+1][b] = stealtharchColor(stealtharch) + Fore.BLACK + 'AR'
        map_matrix[a][b+1] = stealtharchColor(stealtharch) + Fore.BLACK + '  '
        map_matrix[a+1][b+1] = stealtharchColor(stealtharch) + Fore.BLACK + 'CH'
    ...
    for hl in healers:
        a= 2*hl.position[0]
        b = 2*hl.position[1]
        map_matrix[a][b] = blColor(hl) + Fore.BLACK + 'HE'
        map_matrix[a+1][b] = blColor(hl) + Fore.BLACK + 'ER'
        map_matrix[a][b+1] = blColor(hl) + Fore.BLACK + 'AL'
        map_matrix[a+1][b+1] = blColor(hl) + Fore.BLACK + '  '
