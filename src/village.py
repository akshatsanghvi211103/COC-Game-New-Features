...
class Village:
    ...
    def generate_map(self):
        map = np.empty(self.dimensions, dtype="<U10") # <U10 is unicode string of max length 10
    ...
    def check_if_game_over(self, King):
        ...
        if len(all_buildings) == 0 and self.town_hall_obj == None:
            if self.level != 3:
                ...
                return 2
        elif King.alive == False and len(barbarians) == 0 and len(dragons) == 0 and len(balloons) == 0 and len(archers) == 0 and len(stealtharchers) == 0:
            return 3
        else:
            return 0
