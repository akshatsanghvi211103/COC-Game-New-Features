...
barbarians = [] # array of barbarian objects
dragons = []
balloons = []
healers = []
archers = []
...
class Archer:
    ...
    def isInAttackradius(self,pos): # "pos" is the position of the target and "position" is the position of the archer
        r = abs(pos[0] - self.position[0])
        c = abs(pos[1] - self.position[1])
    ...
    def move(self, pos, V, type):
        ...
        if(self.isInAttackradius(pos)):
            ...
            if(flag == 0):
                return
        if(r == 0): # if the target is in the same row
            if(pos[1] > self.position[1]):
                for i in range(self.speed):
                    r = self.position[0]
                    c = self.position[1] + 1 # gotta move right
                    if(self.check_for_walls(r, c, vmap)):
                        self.break_wall(r, c, V)
                        return
                    self.position[1] += 1
    ...
    def deal_damage(self, hit): # hit is the damage dealt by the enemy
        if(self.alive == False):
            return
        self.health -= hit
        if self.health <= 0:
...
class Healer:
    ...
    def isInAttackradius(self,pos): # "pos" is the position of the target and "position" is the position of the archer
        r = abs(pos[0] - self.position[0])
        c = abs(pos[1] - self.position[1])
        if(r**2 + c**2 + 0.1 <= self.attack_radius**2): # 0.1 for decimal errors if they could occur
            return True
        return False
    ...
    def move(self, troop, V):
        ...
        r = abs(pos[0] - self.position[0])
        c = abs(pos[1] - self.position[1])
        # king needs to be handled differently
        troops = barbarians + archers + balloons + stealtharchers + dragons + kingArr
        if(self.isInAttackradius(pos)):
            for troop in troops: # here also handle king seperately
                if (troop.alive == False):
                    continue
                if (abs(troop.position[0] - pos[0]) <= self.heal_range and abs(troop.position[1] - pos[1]) <= self.heal_range):
...
def move_stealtharchers(V, type):
    if(type == 1):
        for stealtharcher in stealtharchers:
            ...
            if(stealtharcher.target == None):
                stealtharcher.target = search_for_closest_building(stealtharcher.position, V.map, 0)
            if(stealtharcher.target == None):
                continue
        ...
        for stealtharcher in stealtharchers:
            if(stealtharcher.alive == False):
                continue
            closest_building = search_for_closest_building(stealtharcher.position, V.map, 0)
            if(closest_building == None):
                continue
...
def move_healers(V):
    for healer in healers:
        if(healer.alive == False):
            continue
        closest_troop = search_for_closest_troop(healer.position)
        if(closest_troop == None):
            continue
        healer.move(closest_troop, V)
...
def search_for_closest_troop(pos):
    troops = barbarians + archers + stealtharchers + dragons + balloons + kingArr
    closest_troop = None
    closest_dist = 10000
    flag = 0
    for troop in troops:
        if(troop.alive == False or troop.health == troop.max_health):
            continue
        dist = (troop.position[0] - pos[0])**2 + (troop.position[1] - pos[1])**2
        if(dist < closest_dist):
            flag = 1
            closest_dist = dist
            closest_troop = troop
