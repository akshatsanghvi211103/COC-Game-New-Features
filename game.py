...
from characters import spawnBarbarian, move_barbarians, spawnArcher, move_archers, spawnStealthArcher, move_stealtharchers, spawnDragon, move_dragons, spawnBalloon, move_balloons, move_healers, spawnHealer, clearTroops
from buildings import shoot_cannons, shoot_wizard_towers, stealtharchers
...
while(True):
    ...
    for stealtharcher in stealtharchers:
        timeVal = time.time()
        if timeVal - stealtharcher.spawnTime > 10 and stealtharcher.invisible == True:
            stealtharcher.invisible = False
    ...
    if ch == 'w':
        ...
        spawnStealthArcher(V.spawn_points[0])
    elif ch == 'g':
        spawnStealthArcher(V.spawn_points[1])
    elif ch == 'h':
        spawnStealthArcher(V.spawn_points[2])
    elif ch == 'q':
        print('quit')
        break
    # os.system('clear')
    ...
    move_stealtharchers(V,pt.movement)
    move_dragons(V)
    move_balloons(V)
    move_healers(V) # after moving the other troops
